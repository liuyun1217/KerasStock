...
if __name__ == '__main__':
    ...
    for i in range(k):
        ...
        test_mae_score, test_mape_score,test_mse_score = model.evaluate(val_data, val_targets)
        print("历史预测误差MAE：" + str(test_mae_score))
        print("历史预测误差百分比MAPE：" + str(test_mape_score))
        print("历史预测均方差MSE：" + str(test_mse_score))
        mae_history = history.history['mape']
        all_mae_histories.append(mae_history)
        all_scores.append(test_mape_score)
        #break
        res = model.predict(predict_data)
        all_predict_res[str(res)] = test_mape_score

    #找出表现最好（mae最小）的模型
    min_mae = min(all_predict_res.values())
    print('挑选出最好的模型误差百分比MAPE为 %f' %min_mae)
    #用表现最好的模型去预测下一日的收盘价
